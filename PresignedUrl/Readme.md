...
WORKDIR /src
COPY ["PresignedUrl.csproj", "PresignedUrl/"]
RUN dotnet restore "PresignedUrl/PresignedUrl.csproj"

WORKDIR "/src/PresignedUrl"
COPY . .
RUN dotnet build "PresignedUrl.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "PresignedUrl.csproj" \
...
Execute unit tests
```
    cd "PresignedUrl/test/PresignedUrl.Tests"
    dotnet test
```

Deploy application
```
    cd "PresignedUrl/src/PresignedUrl"
